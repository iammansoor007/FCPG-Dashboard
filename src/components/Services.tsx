...
// Tailwind JIT Safelist to ensure dynamic database classes are compiled:
const tailwindJitSafelist = [
  "min-h-[340px]",
  "md:min-h-[440px]",
  "lg:row-span-2",
  "min-h-[260px]",
...
function FullBleedCard({
  ...
  className = "", contentPadding = "p-7", ctaLabel = "Explore Service"
}: FullBleedCardProps) {
    ...
    >
      ...
      {imageSrc && (
        <Image
          ...
          className="object-cover transition-transform duration-[1400ms] ease-out group-hover:scale-[1.07] z-0"
          priority={index === 1}
        />
      )}

      {/* Dark gradient — heavier at bottom for legibility, lightens toward top */}
      <div className="absolute inset-0 bg-gradient-to-t from-[#021321]/[0.88] via-[#021321]/[0.35] to-transparent z-10 pointer-events-none" />

      {/* Subtle hover tint overlay */}
      <div className="absolute inset-0 bg-[#031b31]/0 group-hover:bg-[#031b31]/[0.15] transition-colors duration-700 z-10 pointer-events-none" />
...
export default function Services() {
  ...
  const sh = {
    badge: servicesData?.badge || "Our Services",
    heading1: servicesData?.headline?.prefix || "Property Management",
    heading2: servicesData?.headline?.highlight || "Solutions That",
    heading3: servicesData?.headline?.suffix || "Perform",
    description: servicesData?.description || "From residential communities to commercial assets, we deliver expertise, transparency, and peace of mind at every step.",
    footerStrip: servicesData?.footerStrip || "Serving homeowners, investors & developers across South Carolina",
    ctaLabel: servicesData?.ctaLabel || "Explore Service",
    trustPills: (servicesData?.trustPills as any[]) || [
    ...
    >
      ...
      <div className="mx-auto w-full max-w-[1160px] px-6 lg:px-8 py-10 lg:py-14 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-10">
          ...
          >
            <p className="text-[14px] text-text-slate leading-relaxed font-sans font-medium">
              {sh.description}
            </p>
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
              {sh.trustPills.map(({ icon, label }) => {
